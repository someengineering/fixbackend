...
from fixbackend.ids import NotificationProvider, ReportSeverity, WorkspaceId, BenchmarkName
...
async def test_alerting_rule_repo(async_session_maker: AsyncSessionMaker) -> None:
    ...
    slack = AlertingSetting(severity=ReportSeverity.info, channels=[NotificationProvider.slack])
    pd = AlertingSetting(severity=ReportSeverity.critical, channels=[NotificationProvider.pagerduty])
    a = BenchmarkName("a")
    b = BenchmarkName("b")
    c = BenchmarkName("c")
    # insert
