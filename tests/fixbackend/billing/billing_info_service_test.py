...
async def test_list_payment_methods(
    ...
    assert workspace.current_product_tier() == ProductTier.Trial
...
async def test_update_billing(
    ...
    with pytest.raises(NotAllowed) as exc_info:
        ...
        assert "downgrade_not_allowed" in str(exc_info.value)

    # if we change the payment then we still can upgrade to a paid plan
    workspace = await billing_entry_service.update_billing(
        user.id,
        workspace,
